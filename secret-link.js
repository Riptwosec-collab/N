(() => {
  ...
  style.textContent = `
    #secretPortalTrigger{position:fixed;right:10px;bottom:8px;z-index:90;width:26px;height:22px;padding:0;border:0;background:transparent;color:#86cfff;opacity:.10;font:700 13px/1 ui-monospace,Consolas,monospace;cursor:pointer;transition:opacity .18s ease,filter .18s ease;user-select:none}
    ...
    .secret-portal-title{font-weight:800;color:#eaf9ff;font-size:13px}.secret-portal-close{border:0;background:transparent;color:#75a2b9;font-size:19px;cursor:pointer;padding:2px 5px}
    ...
    .secret-portal-btn{min-height:34px;display:inline-flex;align-items:center;justify-content:center;text-decoration:none;border-radius:7px;border:1px solid #176c95;background:#041321;color:#eafaff;padding:6px 11px;font-weight:800;font-size:10px;cursor:pointer}
    .secret-portal-btn.primary{border-color:#1dd5ff;background:linear-gradient(180deg,#08bfff,#0786db);color:#fff}
    .secret-portal-status{min-height:17px;margin-top:8px;color:#ff9eaa;font-size:10px}.secret-portal-status.ok{color:#67e8b0}
    #secretPortalUnlocked{display:none}#secretPortalUnlocked.show{display:block}#secretPortalUnlockArea.hide{display:none}
  `;
  ...
  overlay.innerHTML = `
    <div class="secret-portal-card" role="dialog" aria-modal="true" aria-labelledby="secretPortalTitle">
      <div class="secret-portal-head"><div id="secretPortalTitle" class="secret-portal-title">Private Link</div><button class="secret-portal-close" type="button" aria-label="ปิด">×</button></div>
      <div id="secretPortalUnlockArea">
        ...
        <div class="secret-portal-actions"><button id="secretPortalUnlockBtn" class="secret-portal-btn primary" type="button">ยืนยันรหัส</button><button class="secret-portal-btn secret-portal-cancel" type="button">ยกเลิก</button></div>
      ...
      <div id="secretPortalUnlocked">
        ...
        <div class="secret-portal-actions"><a id="secretPortalOpen" class="secret-portal-btn primary" href="${SECRET_URL}" target="_blank" rel="noopener noreferrer">เข้า Link Work</a><button id="secretPortalCopy" class="secret-portal-btn" type="button">Copy Link</button><button class="secret-portal-btn secret-portal-cancel" type="button">ปิด</button></div>
  ...
  function resetAndClose(){overlay.classList.remove('show');pinInput.value='';status.textContent='';copyStatus.textContent='';unlockArea.classList.remove('hide');unlocked.classList.remove('show')}
  ...
  async function unlock(){status.textContent='';if(await sha256(pinInput.value.trim())!==PIN_SHA256){status.textContent='รหัสไม่ถูกต้อง';pinInput.select();return}unlockArea.classList.add('hide');unlocked.classList.add('show')}
  async function copySecretLink(){try{await navigator.clipboard.writeText(SECRET_URL)}catch{const t=document.createElement('textarea');t.value=SECRET_URL;document.body.appendChild(t);t.select();document.execCommand('copy');t.remove()}copyStatus.textContent='Copy Link แล้ว'}

  trigger.addEventListener('click',()=>{overlay.classList.add('show');setTimeout(()=>pinInput.focus(),30)});
  overlay.querySelector('#secretPortalUnlockBtn').addEventListener('click',unlock);
  pinInput.addEventListener('keydown',e=>{if(e.key==='Enter')unlock();if(e.key==='Escape')resetAndClose()});
